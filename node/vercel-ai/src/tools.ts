...
export function animaTools(config: AnimaToolsConfig = {}) {
	...
	return {
		...
		freeze_card: tool({
			description: "Freeze a virtual card to temporarily block all transactions",
			parameters: z.object({
				cardId: z.string().describe("The card ID to freeze"),
			}),
			execute: async ({ cardId }) => {
				await client.cards.freeze({ cardId });
				return { cardId, status: "frozen" };
			},
		}),
		...
		unfreeze_card: tool({
			...
			parameters: z.object({
				cardId: z.string().describe("The card ID to unfreeze"),
			}),
			execute: async ({ cardId }) => {
				await client.cards.unfreeze({ cardId });
		...
		list_transactions: tool({
			...
			parameters: z.object({
				cardId: z.string().describe("The card ID to list transactions for"),
				limit: z.number().optional().default(20).describe("Max number of transactions"),
			}),
			execute: async ({ cardId, limit }) => {
				const txns = await client.cards.listTransactions({ cardId, limit });
